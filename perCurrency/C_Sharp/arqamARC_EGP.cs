...
//============================================================================================================
{
    ...
    Triplet = Triplet % 100;
    string SubjectName = space + (hasDecimal ? "قرش":"جنيه");               // Default Subject Name is at Pos 1
    if (Triplet > 10)  return space + (hasDecimal ? "قرشاً":"جنيهاً");     // Subject name with Tanween for 11-99
    if (Triplet > 2)   return space + (hasDecimal ? "قروش":"جنيهات");    // Subject name Plural for 3-10
    if (Triplet ==2)   return (hasDecimal ? "قرشان":"جنيهان")+" اثنان"; // Reverse names for 2
    if (Triplet ==1)   return (hasDecimal ?"قرش":"جنيه")+" واحد";        // Reverse names for 1
