...
//============================================================================================================
{
    ...
    int Triplet=0, currencyDigits=3;                                    // number of digits for the currency (i.e. sub unit) change for others e.g. 3 for Kuwaiti Dinar
    ...
    Triplet = Triplet % 100;
    string SubjectName = space + (hasDecimal ? "فلس":"دينار كويتي");               // Default Subject Name is at Pos 1
    if (Triplet > 10)  return space + (hasDecimal ? "فلساً":"ديناراً كويتياً");     // Subject name with Tanween for 11-99
    if (Triplet > 2)   return space + (hasDecimal ? "فلوس":"دنانير كويتية");    // Subject name Plural for 3-10
    if (Triplet ==2)   return (hasDecimal ? "فلسان":"ديناران كويتيان")+" اثنان"; // Reverse names for 2
    if (Triplet ==1)   return (hasDecimal ?"فلس":"دينار كويتي")+" واحد";        // Reverse names for 1
